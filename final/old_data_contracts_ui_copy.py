...
**CRITICAL**: After any tool creates output for human review, use display_csv to show it in the UI.
- After consolidate_contract: display the master contract
- After compare_contracts: display the delta file (if new rules found)
- After merge_and_highlight: ALWAYS display the merged contract
...
def create_ui():
    ...
    with gr.Blocks(
        ...
        def user_submit(message, history, uploaded_files):
            ...
            # Clear uploaded files state (they've been communicated to agent)
            return "", new_history, []
        ...
        def download_artifact(artifacts, active_idx):
            ...
            if csv_data:
                temp_path = os.path.join(tempfile.gettempdir(), f"{artifact['title']}.csv")
        ...
        msg_input.submit(
            ...
            outputs=[msg_input, chatbot, uploaded_files_state],
            queue=False
        ).then(
            bot_response,
