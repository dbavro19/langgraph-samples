...
def process_all_business_terms(df: pd.DataFrame, 
                                ...
                                all_terms: List[str]) -> List[Dict[str, Any]]:
    """
...
def create_master_contract(all_results: List[Dict[str, Any]]) -> tuple:
    """
...
def main():
    ...
    # STEP 1: Load data
    df = load_consumer_contract('test_contract.csv')
    ...
    master_df, audit_df = create_master_contract(all_results)
    
    # STEP 5: Save outputs
    summary = save_outputs(master_df, audit_df)
    ...
    if summary['low_confidence_consolidations'] > 0:
        print(f"\n⚠️  {summary['low_confidence_consolidations']} consolidations flagged for review (confidence < 0.9)")
        print(f"   Review in: output/consolidation_audit_formatted.xlsx (highlighted rows)")
        print(f"   Or filter: output/consolidation_audit.csv where needs_review=TRUE")
